    ...
    const fetchUsers = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch users: ${response.status} ${response.statusText}. Response: ${errorText}`)
        }
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                users.map((user) => {
                  const userName =
                    ((user.firstName || "") + " " + (user.lastName || "")).trim() || user.email || user.firebaseUid
                  return (
                    <TableRow key={user.firebaseUid}>
                      {" "}
                      {/* Key by firebaseUid for consistency */}
                      ...
                      <TableCell>
                        <Badge
                          variant={
                            user.role === "ADMIN" ? "default" : user.role === "SUPERADMIN" ? "destructive" : "secondary"
                          }
                        >
                          {user.role === "ADMIN"
                            ? "Admin"
