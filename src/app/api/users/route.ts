...
export async function GET() {
  try {
    ...
    if (error.message?.toLowerCase().includes("fetch failed")) {
      ...
      errorDetails = 'Error connecting to database: fetch failed. Check DATABASE_URL, NeonDB status (is it paused?), and IP allow-lists in Neon project settings.';
    } else if (error.message?.toLowerCase().includes('relation "users" does not exist')) {
        errorMessage = 'Database Table Missing';
        errorDetails = "The required 'users' table was not found in the database. Ensure migrations (npx prisma migrate dev) have run. Original error: " + error.message;
...
export async function POST(request: Request) {
  try {
    ...
    if (!userData.firebaseUid || !userData.email || !userData.role || !userData.username || !userData.firstName || !userData.lastName) {
      return NextResponse.json({ error: "Missing required user data: firebaseUid, username, email, firstName, lastName, or role." }, { status: 400 });
    }
    ...
    return NextResponse.json({ message: 'User profile stored successfully', userId: result[0].id, firebaseUid: result[0].firebase_uid }, { status: 201 });
    ...
    if (error.message?.toLowerCase().includes("fetch failed")) {
      errorMessage = 'Database Connection Error';
      errorDetails = 'Error connecting to database during user creation: fetch failed. Check DATABASE_URL and NeonDB status.';
    } else if (error.message?.toLowerCase().includes('relation "users" does not exist')) {
        errorMessage = 'Database Table Missing';
        errorDetails = "The required 'users' table was not found in the database. Ensure migrations (npx prisma migrate dev) have run. Original error: " + error.message;
    } else if (error.message?.match(/column ".*" does not exist/i)) {
    ...
    } else if (error.message?.toLowerCase().includes('unique constraint') || error.message?.toLowerCase().includes('duplicate key')) {
        errorMessage = 'Duplicate Entry';
        let field = 'identifier';
        if (error.message?.includes('users_username_key') || error.message?.includes('users_username_unique')) field = 'username';
        if (error.message?.includes('users_email_key') || error.message?.includes('users_email_unique')) field = 'email';
        if (error.message?.includes('users_firebase_uid_key') || error.message?.includes('users_firebase_uid_unique')) field = 'Firebase UID';
